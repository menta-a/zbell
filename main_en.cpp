...
class AshEzsp {
    ...
    // ---- Internal event loop: drains callbacks until deadline ----
    // (copies the logic from v2_zigbee_reader.cpp which DOES work)
        ...
        while (nowMs() < deadline && !g_stop) {
            ...
            if (opt->type == AshType::Data) {
                ...
                if (opt->frm == rxSeq_) {
                    ...
                    sendAck(); // Duplicate ACK
                }
            } else if (opt->type == AshType::Ack) {
                // ACK received from our command - ignore
    ...
    void printIncomingMessage(const std::vector<uint8_t>& p) {
        if (p.size() < 12) {
            dbg() << "  [incomingMessage v13 short] " << hex(p) << "\n";
            return;
        }
    ...
    void decodeReadRsp(uint16_t cluster, uint16_t sender, const std::vector<uint8_t>& msg) {
        ...
        if (volt || amp || watt || en || s) {
            ...
            if (volt) std::cout << " voltage=" << *volt << "V";
            if (amp)  std::cout << " current=" << *amp << "A";
            if (watt) std::cout << " power=" << *watt << "W";
            if (en)   std::cout << " energy=" << *en << "kWh";
            if (s)    std::cout << " state=" << (*s ? "ON" : "OFF");
            std::cout << "\n";
    ...
    bool formNetwork() {
        ...
        if (!euiRsp || euiRsp->payload.size() < 8) {
            std::cerr << "formNetwork: could not read EUI64 from NCP\n";
            return false;
        }
        ...
        if (!secRsp || secRsp->payload.empty() || secRsp->payload[0] != 0x00) {
            std::cerr << "setInitialSecurityState failed\n";
            return false;
        }
        ...
        if (ns && !ns->payload.empty() && ns->payload[0] == 0x02) {
            netUp_ = true;
            std::cout << "  Network formed and JOINED_NETWORK.\n";
            return true;
        }

        std::cerr << "formNetwork: network did not enter JOINED_NETWORK\n";
        return false;
    }

    // ---- EZSP command (copy from v2_zigbee_reader.cpp) ----
    ...
    std::optional<EzspRsp> parseEzsp(const std::vector<uint8_t>& ez, std::optional<uint8_t> ws) {
        ...
        // Unexpected callback - process anyway
        processEzsp(ez);
        return std::nullopt;
    }
    ...
    void commandNoFail(uint16_t code, const std::vector<uint8_t>& payload, const std::string& label) {
        ...
        if (!rsp) {
            dbg() << label << ": no response, continuing\n";
            return;
        }
        if (!rsp->payload.empty()) {
            dbg() << label << " status/response: " << hex(rsp->payload) << "\n";
        } else {
            dbg() << label << " OK\n";
        }
    }
...
public:
    ...
    // NCP Configuration -------------------------------------------------------
    // Equivalent to bellows write_config() - configures essential NCP values
    ...
    void writeNcpConfig() {
        dbg() << "\n  [Configuring NCP (write_config equivalent)]...\n";
        ...
        // CONFIG_PACKET_BUFFER_COUNT must be last
    ...
    bool init() {
        ...
        // CONFIGURE NCP (like bellows write_config)
        writeNcpConfig();
        ...
        dbg() << "  Waiting for NETWORK_UP (10 seconds)...\n";
        processEvents(10000);
        
        // Configure Policies and keys AFTER the network is up (like bellows)
        ...
        if (!netUp_) {
            dbg() << "  [No network. Trying to form new Zigbee network...]\n";
            if (!formNetwork()) {
                std::cerr << "  Error: could not form the Zigbee network.\n";
                return false;
            }
        } else {
            dbg() << "  Network IS UP.\n";
        ...
        dbg() << "\n  [Sending permitJoining(180)]...\n";
        auto perm = cmd(CMD_PERMIT_JOINING, {180}, 3000);
        if (perm && !perm->payload.empty()) {
            dbg() << "  permitJoining response: " << hex(perm->payload) << "\n";
        }

        std::cout << "\n  Network open for 180 seconds.\n";
        std::cout << "  Press pairing button on smart plug (~5s).\n";
        dbg() << "  Waiting for pairing callbacks for 30 seconds...\n";
        dbg() << "  [VERBOSE: logging all callbacks]...\n";
        processEvents(30000);
    ...
    void loop() {
        ...
        while (!g_stop) {
            ...
            // Poll every 5s if there are devices
            int64_t now = nowMs();
